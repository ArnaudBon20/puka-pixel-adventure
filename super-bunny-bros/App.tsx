...
const makeLevel = (): { platforms: Platform[]; coins: Coin[]; enemies: Enemy[] } => {
  const platforms: Platform[] = [
    { x: 0, y: 470, w: WORLD_WIDTH, h: 70, style: 'ground' },
...
const App: React.FC = () => {
  ...
  const statusText = useMemo(() => {
    if (won) return 'Niveau termine. Le lapin bleu a gagne!';
    if (gameOver) return 'Perdu. Essaie encore.';
    return 'Fleches: bouger | Espace: sauter';
  }, [gameOver, won]);
  ...
  useEffect(() => {
    ...
    const { platforms, coins, enemies } = makeLevel();
    ...
    const drawBackground = (): void => {
      ...
      for (let i = 0; i < 5; i += 1) {
        const cloudX = ((cameraX * 0.15 + i * 230) % (WIDTH + 180)) - 90;
        const y = 70 + (i % 3) * 45;
    ...
    const step = (): void => {
      ...
      cameraX = clamp(player.x - WIDTH * 0.4, 0, WORLD_WIDTH - WIDTH);
